...
# Mevcut kodunuzun EN SONUNA bu kodu ekleyin

# Excel export için gerekli import (dosya başında da ekleyebilirsiniz)
try:
    import pandas as pd
    PANDAS_AVAILABLE = True
    print("✅ Pandas kütüphanesi yüklü")
except ImportError:
    PANDAS_AVAILABLE = False
    print("❌ Pandas yüklü değil. Kurulum: pip install pandas openpyxl")

# RelayFaultAnalyzer sınıfına Excel export metodu ekle
def export_to_excel(self, fault_info, protection_data, analysis, filename=None):
    """Analiz sonuçlarını Excel dosyasına aktar"""
    if not PANDAS_AVAILABLE:
        print("❌ Excel export için pandas gerekli! pip install pandas openpyxl")
        return None
        
    if filename is None:
        filename = f"rele_ariza_analizi_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
        ...
        with pd.ExcelWriter(filename, engine='openpyxl') as writer:
            ...
            general_data = {
                'Parametre': [
                    ...
                    'IL1 Anlık Değer', 'IL1 Etkin Değer', 'Rapor Zamanı'
                ],
                'Değer': [
                    ...
                    fault_info.get('cursor_values', {}).get('IL1_instant', 'Bilinmiyor'),
                    fault_info.get('cursor_values', {}).get('IL1_rms', 'Bilinmiyor'),
                    datetime.now().strftime('%d.%m.%Y %H:%M:%S')
                ]
            }
            pd.DataFrame(general_data).to_excel(writer, sheet_name='Genel Bilgiler', index=False)
            
            # 2. Koruma Fonksiyonları Sayfası
            if protection_data:
                protection_df = pd.DataFrame([
                    {
                        ...
                        'Durum': p['status'],
                        'Kaynak': p['line'][:50] + '...' if len(p['line']) > 50 else p['line']
                    }
                    for i, p in enumerate(protection_data)
                ])
                protection_df.to_excel(writer, sheet_name='Koruma Fonksiyonları', index=False)
            
            # 3. Koruma İstatistikleri
            if protection_data:
                # Koruma türleri sayımı
                protection_types = {}
                status_counts = {}
                ...
                stats_df = pd.DataFrame([
                    {'Kategori': 'Koruma Türleri', 'Öğe': k, 'Adet': v}
                    for k, v in protection_types.items()
                ] + [
                    {'Kategori': 'Koruma Durumları', 'Öğe': k, 'Adet': v}
            ...
            # 5. Simülasyon Verileri (Grafikler için Excel'de kullanabilirsiniz)
            time_points = np.linspace(0, 2, 100)
            il1_data = 150 * np.sin(2*np.pi*50*time_points) * np.exp(-time_points/0.5)
            il2_data = 145 * np.sin(2*np.pi*50*time_points + 2*np.pi/3) * np.exp(-time_points/0.5)
            il3_data = 155 * np.sin(2*np.pi*50*time_points - 2*np.pi/3) * np.exp(-time_points/0.5)
            ...
            simulation_df = pd.DataFrame({
                ...
                'IL2 Akım (A)': il2_data,
                'IL3 Akım (A)': il3_data,
                'Pickup Noktası': ['0.1s' if abs(t - 0.1) < 0.01 else '' for t in time_points],
                'Trip Noktası': ['0.3s' if abs(t - 0.3) < 0.01 else '' for t in time_points]
            })
            simulation_df.to_excel(writer, sheet_name='Grafik Verileri', index=False)
            
            # 6. Koruma Sırası
            if analysis.get('protection_sequence'):
                sequence_df = pd.DataFrame([
                    {
                        'Sıra': seq['order'],
                        'İşlem': seq['action'],
                        'Koruma': seq['protection'],
                        'Durum': seq.get('status', 'N/A')
                    }
                    for seq in analysis['protection_sequence']
                ])
                sequence_df.to_excel(writer, sheet_name='Koruma Sırası', index=False)
        
        print(f"✅ Excel dosyası başarıyla oluşturuldu: {filename}")
        print(f"📁 Dosya konumu: {os.path.abspath(filename)}")
        return filename
...
# Metodu RelayFaultAnalyzer sınıfına ekle
RelayFaultAnalyzer.export_to_excel = export_to_excel

# Mevcut analyze_pdf_complete metodunu güncelleyin
# Orijinal metodun sonuna şu satırları ekleyin:

def analyze_pdf_complete_updated(self, pdf_path, export_excel=True):
    """PDF'i tam analiz et - metin + görüntü + Excel export"""
    print("🔍 PDF analizi başlatılıyor...")
    ...
    if raw_text:
        ...
        # BURAYA YENİ: Excel export
        excel_file = None
        if export_excel:
            print("\n📊 Excel dosyası oluşturuluyor...")
            excel_file = self.export_to_excel(fault_info, protection_data, analysis)
        ...
        return {
            ...
            'analysis': analysis,
            'report': report,
            'excel_file': excel_file  # YENİ ALAN
        }
    else:
        print("❌ PDF'den metin çıkarılamadı!")
        return None
...
RelayFaultAnalyzer.analyze_pdf_complete = analyze_pdf_complete_updated

# Ana kullanım fonksiyonunu da güncelleyin
def analyze_relay_fault_from_pdf_with_excel(pdf_path, export_excel=True):
    """PDF dosyasından röle arıza kaydını tam analiz et ve Excel'e aktar"""
    analyzer = RelayFaultAnalyzer()
    return analyzer.analyze_pdf_complete(pdf_path, export_excel)
...
if __name__ == "__main__":
    ...
    print("-" * 50)
    print("🚀 Röle Arıza Analizi + Excel Export Başlatılıyor...")
    print(f"📁 Dosya: {pdf_file_path}")
    print("-" * 50)
    # PDF'i analiz et
    try:
        results = analyze_relay_fault_from_pdf(pdf_file_path)
        results1 = analyze_relay_fault_from_pdf_with_excel(pdf_file_path, export_excel=True)
        ...
        if results1:
            print("\n✅ Analiz başarıyla tamamlandı!")
            print(f"📄 Çıkarılan metin uzunluğu: {len(results1['extracted_text'])} karakter")
            print(f"🛡️ Tespit edilen koruma sayısı: {len(results1['protection_data'])}")
            print(f"💡 Önerilen çözüm sayısı: {len(results1['analysis']['recommendations'])}")
            
            # Excel dosyası bilgisi
            if results1['excel_file']:
                print(f"📊 Excel dosyası: {results1['excel_file']}")
                print(f"📍 Dosya konumu: {os.getcwd()}")
            else:
                print("⚠️ Excel dosyası oluşturulamadı")
        else:
            print("\n❌ PDF'den metin çıkarılamadı veya analiz yapılamadı!")
